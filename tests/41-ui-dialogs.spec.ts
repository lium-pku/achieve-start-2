...
test.describe('UI 41：Dialog 表单交互', () => {
  ...
  test.describe('活动对话框', () => {
    ...
    test('打开后默认有"活动名称"输入框和"保存/取消"按钮', async ({ page }) => {
    ...
    test('点取消关闭对话框，不创建活动', async ({ page }) => {
      await page.getByPlaceholder('例如：阅读 20 分钟').fill('不应该被创建的活动')
      await page.getByRole('button', { name: '取消' }).click()
      await expect(page.getByText('新建活动')).toHaveCount(0)
    })

    test('切换周期类型为 weekly，应显示"每周哪一天"选择器', async ({ page }) => {
      await page.locator('button[role="combobox"]').first().click()
      await page.getByText('周度').click()
      await expect(page.getByText('每周哪一天')).toBeVisible()
    })

    test('切换周期类型为 monthly，应显示"每月几号"', async ({ page }) => {
      await page.locator('button[role="combobox"]').first().click()
      await page.getByText('月度').click()
      await expect(page.getByText('每月几号')).toBeVisible()
    })

    test('切换周期类型为 once，应显示"具体日期"', async ({ page }) => {
      await page.locator('button[role="combobox"]').first().click()
      await page.getByText("临时（一次性）").click()
  ...
  test.describe('奖励对话框', () => {
    test.beforeEach(async ({ page }) => {
      ...
      await expect(page.getByRole("button", { name: /新建奖励/ })).toBeVisible({ timeout: 5000 }); await page.getByRole("button", { name: /新建奖励/ }).click()
    ...
    test('未填名称点保存，提示错误且不关闭', async ({ page }) => {
      await page.getByRole('button', { name: '保存' }).click()
      await expect(page.locator('[data-sonner-toast]')).toBeVisible({ timeout: 3000 })
  ...
  test.describe('成员对话框', () => {
    ...
    test('打开后显示姓名、角色、头像、主题色', async ({ page }) => {
      ...
      // 角色/头像/主题色可能需要滚动可见
      await expect(page.getByText('角色', { exact: true })).toBeVisible({ timeout: 5000 })
      await expect(page.getByText('头像', { exact: true })).toBeVisible()
  ...
  test.describe('目标对话框', () => {
    test.beforeEach(async ({ page }) => {
      // 直接以孩子身份登录（比 switchMemberByUI 更可靠）
      await uiLoginFast(page, 'test-child')
      await gotoTab(page, 'planning')
      // 等待规划页加载完成
      await expect(page.getByText('规划').first()).toBeVisible({ timeout: 5000 })
      // "新建目标"按钮只对孩子可见
      await page.waitForTimeout(500)
      await page.getByRole('button', { name: /新建目标/ }).first().click({ timeout: 5000 })
      await expect(page.getByRole('dialog')).toBeVisible({ timeout: 5000 })
    })
    ...
    test('可切换完成状态（未开始/进行中/已达成）', async ({ page }) => {
      await page.locator('button[role="combobox"]').first().click()
      // Radix Select 下拉项在 portal 中渲染，用 role=option 定位
      await expect(page.locator('[role="option"]', { hasText: '进行中' })).toBeVisible({ timeout: 3000 })
      await expect(page.locator('[role="option"]', { hasText: '已达成' })).toBeVisible()
    })
  })
  ...
  test.describe('鼓励里程碑对话框', () => {
    ...
    test('打开后显示阈值、称号、鼓励语、图标', async ({ page }) => {
      await expect(page.getByText('触发积分')).toBeVisible()
      await expect(page.getByText("称号 *")).toBeVisible()
      await expect(page.getByText('鼓励语')).toBeVisible()
      await expect(page.getByText('图标')).toBeVisible()
    })

    test('未填称号/鼓励语点保存，提示错误', async ({ page }) => {
      await page.getByRole('button', { name: '保存' }).click()
      await expect(page.locator('[data-sonner-toast]')).toBeVisible({ timeout: 3000 })
    })

    test('填写完整保存成功', async ({ page }) => {
      await page.getByPlaceholder('例如：进步小达人').fill('测试里程碑-自动化')
      await page.getByPlaceholder('例如：50 分达成，你真棒！').fill('测试鼓励语')
      await page.getByRole('button', { name: '保存' }).click()
      await expect(page.locator('[data-sonner-toast]')).toBeVisible({ timeout: 3000 })
      await expect(page.getByRole('dialog')).toHaveCount(0, { timeout: 5000 })
    })

    test('阈值为 0 点保存，提示错误', async ({ page }) => {
      await page.getByPlaceholder('例如：进步小达人').fill('阈值测试')
      await page.getByPlaceholder('例如：50 分达成，你真棒！').fill('鼓励语')
